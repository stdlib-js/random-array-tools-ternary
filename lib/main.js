...
function Random( prng, dtypes, dtype ) {
	...
	if ( !isFunction( prng ) ) {
		throw new TypeError( format( '1qn3c', prng ) );
	}
	...
	if ( !isStringArray( dtypes ) ) {
		throw new TypeError( format( '1qnAS', dtypes ) );
	}
	...
	if ( !contains( dtypes, dtype ) ) {
		throw new TypeError( format( '1qnFO', dtype ) );
	}
	this._prng = prng;
	this._dtypes = dtypes;
	this._dtype = dtype;
...
setReadOnly( Random.prototype, 'generate', function generate( len, param1, param2, param3, options ) {
	...
	if ( !isNonNegativeInteger( len ) ) {
		throw new TypeError( format( '1qn2d', len ) );
	}
	opts = {};
...
setReadOnly( Random.prototype, 'assign', function assign( param1, param2, param3, out ) {
	if ( !isCollection( out ) ) {
		throw new TypeError( format( '1qn2l', out ) );
	}
